...
Yeah, aik NoSQL aur non-relational database hey, jo data ko aik JSON formate mey store karta hey jisko ham BSON (Binary JSON) bulatey heyn.

**Schema Flexibility** mongo db mey SQL database ki tarah schema fixed nahi hota yani agar mey pehlay user ka name or age store karwa raha to abh mey uska gender bhi store karwa skta hun, which is good for rapidly changing scenarios.
...
Collection k ander har JSON entry ko ham document kehtay heyn. Aik document aik JSON ki taraha ka tarah ka object hota hey jismey hamari key value p mairey data store kartey heyn
